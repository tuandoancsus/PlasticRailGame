...
public class Camera
{
	 ...
	 *
	 * @param direction The amount to move forward.  A positive value moves the object forward,
	 *                  a negative value moves it backward.
	 */
	public void fwdAction(float direction) {
		Vector3f fwdDirection = new Vector3f(getN().mul(direction));
		fwdDirection.mul(0.1f);
		setLocation(getLocation().add(fwdDirection.x(), fwdDirection.y(), fwdDirection.z()));
	}

	/**
	 * Rotates the object around its vertical (Y) axis.
	 *
	 * @param angle The angle of rotation in radians.  A positive value rotates the object
	 *              clockwise (when viewed from above), a negative value rotates it counter-clockwise.
	 */
	public void yaw(float angle) {
		Vector3f rightVector = new Vector3f(getU());
		Vector3f upVector = new Vector3f(getV());
		Vector3f fwdVector = new Vector3f(getN());

		rightVector.rotateAxis(angle, upVector.x(), upVector.y(), upVector.z());
		fwdVector.rotateAxis(angle, upVector.x(), upVector.y(), upVector.z());
	...
	/**
	 * Rotates the object around its horizontal (X) axis (pitch).
	 *
	 * @param angle The angle of rotation in radians. A positive value rotates the object upwards,
	 *             a negative value rotates it downwards.
	 */
	public void pitch(float angle) {
		...
		upVector.rotateAxis(angle, rightVector.x(), rightVector.y(), rightVector.z());
		fwdVector.rotateAxis(angle, rightVector.x(), rightVector.y(), rightVector.z());
	...
	/**
	 * Rotates the object around its forward (Z) axis (roll).
	 *
	 * @param angle The angle of rotation in radians. A positive value rotates the object clockwise
	 *             when looking along the forward vector, a negative value rotates it counter-clockwise.
	 */
	public void roll(float angle) {
		...
		rightVector.rotateAxis(angle, fwdVector.x(), fwdVector.y(), fwdVector.z());
		upVector.rotateAxis(angle, fwdVector.x(), fwdVector.y(), fwdVector.z());
	...
	public void zoom(float zoomAmount) { 
		Vector3f newPosition = getLocation().add(getN().mul(zoomAmount * 0.1f));
		setLocation(newPosition);
	}

	public void panVertical(float amount) {
		// Get the current location of the camera
		Vector3f currentPosition = getLocation();
		
		// Adjust the Y coordinate for vertical panning (up or down)
		Vector3f newPosition = currentPosition.add(0, 0, amount);
	...
	public void panHorizontal(float amount) {
		...
		// Adjust the X coordinate for horizontal panning (left or right)
		Vector3f newPosition = currentPosition.add(amount, 0, 0);
